...
export function TaskListPage(props: TaskListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTasks({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'briefing.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'briefing', header: 'Briefing', accessorKey: 'briefing' },
    hasAccess('project', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('creator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'creator',
          header: 'Creator',
          accessorKey: 'creator',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/creators/view/${record.creator?.id}`}>
              {record.creator?.social_media_credentials}
            </Link>
          ),
        }
      : null,
