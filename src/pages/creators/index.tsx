...
import { getCreators, deleteCreatorById } from 'apiSdk/creators';
import { CreatorInterface } from 'interfaces/creator';

type ColumnType = ColumnDef<CreatorInterface, unknown>;

interface CreatorListPageProps {
...
export function CreatorListPage(props: CreatorListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCreators({
        ...
        order: params.order,
        searchTermKeys: ['social_media_credentials.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CreatorInterface>>(
    () => `/creators?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CreatorInterface) => {
    if (hasAccess('creator', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/creators/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'social_media_credentials', header: 'Social Media Credentials', accessorKey: 'social_media_credentials' },
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/creators/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('creator', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/creators/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('creator', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="creator" />
        </Flex>
        {hasAccess('creator', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/creators/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
